...
const fs = require('fs');
const pluginName = 'mht-dashboard';
...
distFilenames = fs.readdirSync(`${destination}/dist/mht-dashboard/browser`);
...
const updateLine = (line, name) => {
  const matchedLinePart = line.match(/(?<=mht-dashboard\/browser\/).*?(?=\')/gs)?.toString();
